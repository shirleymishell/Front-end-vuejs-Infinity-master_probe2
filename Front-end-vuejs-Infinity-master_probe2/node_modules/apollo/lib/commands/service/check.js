...
🔢 Compared **${utils_1.pluralize(diffToPrevious.changes.length, "schema change")}** against **${utils_1.pluralize(diffToPrevious.numberOfCheckedOperations, "operation")}** seen over the **last ${formatTimePeriod(hours)}**.
...
class ServiceCheck extends Command_1.ProjectCommand {
    async run() {
        ...
        const { checkSchemaResult, config, shouldOutputJson, shouldOutputMarkdown, compositionErrors } = taskOutput;
        if (compositionErrors) {
            if (shouldOutputJson) {
                throw new Error("Federated service check does not yet support JSON output formatting");
            }
            else if (shouldOutputMarkdown) {
                throw new Error("Federated service check does not yet support markdown output formatting");
            }
            console.log("");
